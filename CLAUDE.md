...
|---|---|---|
| — | Intro | The claim — **the site's own question, pointed the way the series point: nine groups, one of them goes first, and the count reaches you late**. Not the canvas's "a year — not a probability": R14 means no year arrives, and an intro that promises one makes the result screen read as broken rather than finished. **Nor the opposite** — "no date is published anywhere" is true and, as a headline, argues the reader is safe, which the result screen contradicts: the share is a net figure and the occupation vintage lags, so a line falls after the fall has reached the people in it. 0010 R5's fourth revision note records that round. One CTA naming what the reader receives (`Find my group →`, not `Start →`), and **no capability chips** — the canvas has three; R5's second revision note records why they are not here |
