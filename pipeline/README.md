...
| `iso2` | World Bank country metadata (`iso2Code`) | ISO 3166-1 alpha-2, carried for the app's country search — `Intl.DisplayNames` needs an alpha-2 to return the name a reader types. **Null for the three `EXTRA_AREAS`**, which are outside the Bank's country list; `TWN` is the live case and is not filled by hand |
