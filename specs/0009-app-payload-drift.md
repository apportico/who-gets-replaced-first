...
- `pipeline/README.md` states that `src/data/global_labor.json` and
  ...
  module.
- The check is written to **fail before R1 lands**: assert the committed
  payload actually carries the 84-entry `field_tiers` block the README's line
  139 promises. The first draft of this requirement asked only that
  `grep -n "field_tiers" pipeline/README.md` return a line, which **passes
  today against the stale artifact** — a vacuous acceptance criterion of
  exactly the kind this spec exists to remove, and it does not become
  acceptable for sitting in a spec rather than a test.
