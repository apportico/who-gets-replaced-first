...
| R5 — touch targets, focus ring | 2026-09-01 | **Closed by computed-style test** (`[~]`). The CTA computes 60px, an option 56px, the tertiary 48px, a band chip 56px, and a sweep over all five screens fails on a *missing* floor rather than skipping it, with a visited-count guard. Residual, browser-only: what those floors paint against real content; whether the three faces load; and **step 03's band grid**, where Radix's `w-fit` collapsed the columns and no suite could see it — jsdom resolves declarations, not layout. |
