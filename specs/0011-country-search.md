...
and `src/data/global_labor.json`; the committed payload carries a non-null
...
R1's options. It matches against, in this order of preference for display but
without ranking beyond it:
...
matching route it exercises, so a regression says which of the four broke.
...
us and why each entry exists. Scope is the reader's short form only — `UK`,
`USA`/`US`, `UAE`, `Turkey`, `Czech Republic`, `Swaziland`, `Ivory Coast`,
`East Timor`, `Holland`. It is not a second name list and never a second
coverage source: an alias may only point at an `iso3` that R1 already returns.

**Acceptance:** Vitest — the table has **at most 15 entries**; every value is an
`iso3` present in `countryOptions(rows)`; no entry duplicates a string
`Intl.DisplayNames` already returns for that row (asserted by computing the
`Intl` name and failing if the alias equals it, so the table cannot silently
grow into work the standard already does).
...
no `official series` string rendered on step 01; 0010 R10's stated-absence test
(`groupFigures.js`, a null group yields the stated-absence branch) still passes
unmodified.
