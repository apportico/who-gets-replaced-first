...
- **Export removed or renamed** (the fifth direction, added after review round 4 found the guard had lost this one): renaming `export const DEADLINE_MS` gives `check-settings FAILED: - pre-push-verify.mjs does not export a numeric DEADLINE_MS`. This direction existed, was briefly lost when the import was wrapped in a `try`/`catch` whose success was *inferred* from `DEADLINE_MS === undefined` — a missing export destructures to `undefined` without throwing, so the branch asserted nothing and the success line printed — and is restored by tracking the import with a boolean. Restored → green.
