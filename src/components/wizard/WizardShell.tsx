...
export default function WizardShell() {
  ...
  // 0019 R17. The ONE URL read that has to survive prerendering, and the only
  // reason this component is a client island. `search()` reads
  // globalThis.location, which does not exist at build time — so the
  // prerendered HTML would be the intro while the first client render produced
  // real state, a hydration mismatch on every stateful URL.
  //
  // useSearchParams is read here, at mount, for `boot` only. Every later
  // transition stays on the useState + history + popstate path below, untouched
  // — which is why the raw pushState in `commit` is not a problem: nothing
  ...
  return (
    ...
    >
      ...
      >
        {/* 0019 R17. The static frame is shared with WizardChrome, the
            Suspense fallback that gets prerendered. Passing `step` is what
            adds the NN/04 counter and the segment fills on top of it. */}
