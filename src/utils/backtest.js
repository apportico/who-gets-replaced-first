...
const SERIES = {}
Object.entries(payload.series).forEach(([iso3, groups]) => {
  SERIES[iso3] = {}
...
const SUMMARY = {}
payload.summary.forEach((values) => {
  const row = hydrate(values, SUMMARY_FIELDS)
  SUMMARY[row.group] = row
})

export const BACKTEST_FIELD_TIERS = payload.field_tiers
...
export function groupColumn(group) {
  return COLUMN[group] ?? null
}

/** The tier a field carries, from the payload's own registry — never guessed. */
export function tierFor(field) {
 ...
 * was never computed.
 */
export function backtestFor(iso3, group) {
...
const pp = (v) => `${v >= 0 ? '+' : '−'}${Math.abs(v).toFixed(2)}pp`
