...
import type { LaborRow, CountryOption } from '@/types'
 ...
 */
export function intlName(iso2: string | null | undefined) {
...
function matchesName(option: CountryOption, q: string) {
  return fold(option.name).includes(q)
}

function matchesCode(option: CountryOption, q: string) {
  return fold(option.iso3).startsWith(q)
}

function matchesIntl(option: CountryOption, q: string) {
...
function matchesAlias(option: CountryOption, q: string) {
...
export function matches(option: CountryOption, query: string) {
  const q = fold(query)
  if (!q) return true
...
export function searchCountries(
  ...
  query: string,
  { limit = MATCH_LIMIT, absentLimit = ABSENT_LIMIT }: { limit?: number; absentLimit?: number } = {},
) {
  const q = fold(query)
