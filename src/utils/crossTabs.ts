...
/** One band as the screen renders it: the published key, its label, its value. */
export interface Band { key: string; label: string; value: number }

/**
 * A band read. `residualNote` is optional because only a PRESENT read has one,
 * and declaring it here is what lets `ageBands`/`eduBands` set it — 0019 R6's
 * point, one level down: the shape says which fields exist in which state.
 */
...
type CrossTabs = { values?: Record<string, unknown> } | null | undefined
...
function readBands(
  ...
  const v: Record<string, unknown> = crosstabs.values ?? {}
  const present: Band[] = bands
    .map((b) => ({ ...b, value: v[`isco${group}_${prefix}_${b.key}_pct`] as number }))
  ...
  if (!present.length) {
    ...
    return {
      ...
      // none to name.
      year: flag === FLAG_WITHHELD ? ((v[`isco${group}_${prefix}_year`] as number) ?? null) : null,
    }
  }
  return {
    state: PRESENT,
    bands: present,
    year: (v[`isco${group}_${prefix}_year`] as number) ?? null,
    tier: 'DERIVED',
...
export function ageBands(crosstabs: CrossTabs, group: number | null | undefined): BandRead {
...
export function eduBands(crosstabs: CrossTabs, group: number | null | undefined): BandRead {
