...
export function groupShare(row: LaborRow | null | undefined, group: number | null | undefined) {
  const g = groupByNumber(group)
  if (!row || !g) return { state: NOT_PUBLISHED, value: null }
  const value = row[g.key] as number | null | undefined
  ...
  return {
    ...
    display: fmt(value, 1, '%'),
    tier: DERIVED,
    year: (row.data_year_occupation as number | null) ?? null,
    label: g.label,
  }
}
 ...
 */
export function groupHeadcount(row: LaborRow | null | undefined, group: number | null | undefined) {
  const g = groupByNumber(group)
  if (!row || !g) return { state: NOT_PUBLISHED, value: null }
  // Narrowed at the read, not asserted at the arithmetic. The payload's index
  // signature yields `unknown` deliberately (0019 R6) so a field cannot be
  // multiplied before anyone has said it is a number.
  const share = row[g.key] as number | null | undefined
  const employed = row.employed_total as number | null | undefined
