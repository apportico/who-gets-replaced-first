...
import type { ShareCardModel } from '@/types'
 ...
 */
export function assertDisplayFace(ctx: CanvasRenderingContext2D) {
  ctx.save()
...
function setLetterSpacing(ctx: CanvasRenderingContext2D, value: string) {
  ...
  try { (ctx as CanvasRenderingContext2D & { letterSpacing: string }).letterSpacing = value } catch { /* older engine, spacing omitted */ }
}
...
function badge(ctx: CanvasRenderingContext2D, text: string, x: number, y: number): number {
 ...
 */
export function drawCard(model: ShareCardModel, canvas?: HTMLCanvasElement | null, scale = SCALE) {
  const c = canvas ?? document.createElement('canvas')
  c.width = CARD_W * scale
  c.height = CARD_H * scale
  const ctx = c.getContext('2d')
  // Throwing beats drawing nothing: a null 2D context means the card silently
  // comes out blank, and a blank card is a card with no tiers on it.
 ...
 */
export function siteCardModel(): ShareCardModel {
  return {
...
export function cardFilename(model: ShareCardModel) {
